...
const byte REPLAY_CLOCK_PIN = 20;  //Adjust as required
const byte REPLAY_DATA_PIN = 21;   //Adjust as required
...
const byte YELLOW_LED_PIN = 3;    //Adjust as required
const byte GREEN_LED_PIN = 4;     //Adjust as required
const byte EXT_PIN = 5;           //Adjust as required spare output

//----- OLED constants ---------------------------------------------------------------------------------------------
const bool OLED = true;  //Will display data to the OLED display if true
...
const byte SCREEN_ADDRESS = 0x3C; ///< See datasheet for Address; 0x3D for 128x64, 0x3C for 128x32
...
// int pinBits = 110;                 //keypad number expected bit count (quick press)
...
DNSServer dnsServer;               //DNS for captive portal
...
void ConfigureOLED() {
    ...
    const unsigned char paxtogeddon_logo [] = {
    ...
    display.drawBitmap(0, 0, paxtogeddon_logo, SCREEN_WIDTH, SCREEN_HEIGHT, WHITE);
    display.display();
}
//----- Configure WiFi ----------------------------------------------------------------------------------------------
void ConfigureWiFi() {
...
void CardReplayGPIO(int noOfBits, String binaryData, String cardNo) {
  ...
  digitalWrite(REPLAY_DATA_PIN, 0);
...
void WebServerTask(void* parameter) {
  ...
  webServer.on("/generate_204", HTTP_GET, [](AsyncWebServerRequest* request) {
    request->redirect(String("http://") + AP_IP.toString());
  });
...
/*
  pinMode(REPLAY_CLOCK_PIN, INPUT);
  pinMode(REPLAY_DATA_PIN, INPUT);
//  pinMode(WIFI_MODE_PIN, INPUT_PULLUP);
