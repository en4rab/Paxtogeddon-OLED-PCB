...
void BLETask(void* p) {
  vTaskDelay(pdMS_TO_TICKS(8000));  // wait for WiFi AP + web server to be fully running
  prBLE::FitProM4Init();            // allocate NimBLE heap after first pages served
  while (true) {
    ...
    if (prSettings::bleTriggerPending && prBLE::IsConnected()) {
      prSettings::bleTriggerPending = false;
      prBLE::TriggerVibrate();
    }
