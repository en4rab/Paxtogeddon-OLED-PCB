...
static void ServerTasks(void* p) {
  ...
  webServer.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
    ...
    response->print(prHTML::Header(autoRefresh, requestedPage));
    response->print(prHTML::Menu(autoRefresh));
    response->print(prHTML::VersionAndBuild());
    response->print(prHTML::LastCardData());
    response->print(prHTML::CardData(autoRefresh, requestedPage, showCloneAction));
    response->print(prHTML::Footer());
  ...
  webServer.on("/sendCardNumber", HTTP_GET, [](AsyncWebServerRequest* request) {
    AsyncResponseStream *response = request->beginResponseStream("text/html");
    response->print(prHTML::Header(false, 1));
    response->print(prHTML::SendCardNumber());
    response->print(prHTML::Footer());
    request->send(response);
  });
  ...
  webServer.on("/confirmClearLog", HTTP_GET, [](AsyncWebServerRequest* request) {
    AsyncResponseStream *response = request->beginResponseStream("text/html");
    response->print(prHTML::Header(false, 1));
    response->print(prHTML::ConfirmMessage("clearLog", "Are you sure you want to clear the log?"));
    response->print(prHTML::Footer());
    request->send(response);
  });
  ...
  //Trigger integrated BLE vibration
  webServer.on("/triggerBleScan", HTTP_GET, [](AsyncWebServerRequest* request) {
    prSettings::lastCardData = "BLE vibrate triggered";
  ...
  webServer.on("/confirmReboot", HTTP_GET, [](AsyncWebServerRequest* request) {
    AsyncResponseStream *response = request->beginResponseStream("text/html");
    response->print(prHTML::Header(false, 1));
    response->print(prHTML::ConfirmMessage("rebootESP32", "Are you sure you want to reboot the ESP32?"));
    response->print(prHTML::Footer());
    request->send(response);
  });
  ...
  webServer.on("/viewSettings", HTTP_GET, [](AsyncWebServerRequest* request) {
    AsyncResponseStream *response = request->beginResponseStream("text/html");
    response->print(prHTML::Header(false, 1));
    response->print(prHTML::ViewSettings(""));
    response->print(prHTML::Footer());
    request->send(response);
  });
  ...
  webServer.on("/saveSettings", HTTP_POST, [](AsyncWebServerRequest* request) {
      ...
      if (hasAllParams) {
        ...
        if (hasAllParams) {
          ...
          if (request->hasParam("bleenabled", true)) {
            AsyncWebParameter* p9 = request->getParam("bleenabled", true);
            if (p9->value() == "on") {
              prSettings::bleEnabled = true;
            }
          }

          if (request->hasParam("blescan", true)) {
            AsyncWebParameter* p10 = request->getParam("blescan", true);
            if (p10->value() == "on") {
              prSettings::bleMAC = "";  // clear saved MAC — forces re-scan on next boot
            }
          }
    ...
    AsyncResponseStream *response = request->beginResponseStream("text/html");
    response->print(prHTML::Header(false, 1));
    response->print(prHTML::ViewSettings(message));
    response->print(prHTML::Footer());
    request->send(response);
  });
  //-------------------------------------------------------------------------------------------------------------------------
