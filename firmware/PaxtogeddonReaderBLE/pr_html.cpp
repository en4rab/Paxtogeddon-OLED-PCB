...
String Menu(bool autoRefresh) {
  ...
  if (!autoRefresh) {
    ...
    html += prSettings::bleEnabled ? "<a href=\"/triggerBleScan\">Trigger BLE Vibrate</a><hr>" : "";
...
String ViewSettings(String message) {
  ...
  html += "</div></td></tr></table>";
  String bleEnabledChecked = prSettings::bleEnabled == true ? "checked" : "";
  String bleMACDisplay = prSettings::bleMAC == "" ? "None" : prSettings::bleMAC;
  html += "<table width=\"100%\" border=\"0\" cellspacing=\"5\" cellpadding=\"0\"><tr>";
  html += "<td align=\"left\" valign=\"top\"><div class=\"infoPanel\">";
  html += "<div class=\"settingsHeader\">BLE Settings (FitPro M4/M5/M6)</div>";
  html += "<div class=\"settingsName\">BLE Enabled:<input type=\"checkbox\" id=\"bleenabled\" name=\"bleenabled\" " + bleEnabledChecked + "></div>";
  html += "<div class=\"settingsName\">Paired Device MAC: " + bleMACDisplay + "</div>";
  html += "<div class=\"settingsName\">Re-scan (clears saved MAC, forces scan on next boot):<input type=\"checkbox\" id=\"blescan\" name=\"blescan\"></div>";
  html += "</div></td></tr></table></form>";
  return html;
}
