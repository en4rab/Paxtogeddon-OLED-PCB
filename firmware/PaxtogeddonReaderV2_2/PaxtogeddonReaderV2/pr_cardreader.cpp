...
static bool CheckBitCount(void) {
  if (bitCount > 0) {
    ...
    //We have more bits than Net2, so assume it's a Net2 Long Range card
    ...
    if (bitCount != net2Bits && bitCount != net2lrBits && bitCount != switch2Bits) {
      prSettings::lastCardData = "Bit count error, received: " + String(bitCount);
      prUtil::OLEDShowMessage("Bit count error", "Recevied", String(bitCount), "Expected", String(net2Bits) + ", " + String(net2lrBits), String(switch2Bits));
